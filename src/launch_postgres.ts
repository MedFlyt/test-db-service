...
export type PostgresVersion = keyof typeof PostgresVersions;

export function readPostgresVersion(postgresVersion: string): PostgresVersion | null {
    if (Object.keys(PostgresVersions).indexOf(postgresVersion) >= 0) {
        return postgresVersion as PostgresVersion;
    } else {
        return null;
    }
}
